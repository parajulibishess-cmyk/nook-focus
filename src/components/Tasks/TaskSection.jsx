...
const TaskSection = ({ tasks, setTasks, focusedTaskId, setFocusedTaskId, todoistToken, setStats, transparent }) => {
    ...
    if (todoistToken) {
        try { 
            ...
            if (res.ok) {
                ...
                // CRITICAL: Swap temp ID with real Todoist ID so subsequent actions (delete/check) work on server
    ...
    setTasks(prev => prev.map(t => {
        if (t.id === id) {
            ...
            if (todoistToken) {
                ...
                fetch(`https://api.todoist.com/rest/v2/tasks/${id}/${action}`, {
                    method: 'POST',
  ...
  const discardTask = async (id) => {
      ...
      if (todoistToken) {
          try {
              ...
              console.error("Failed to delete on Todoist", e);
          }
      }
  };
    ...
    <Card transparent={transparent} className="flex-1 flex flex-col h-full min-h-[350px] overflow-visible relative z-20">
      ...
      <div className="flex-1 overflow-y-auto space-y-3 pr-2 pl-1 custom-scrollbar rounded-2xl relative z-0">
        {tasks.map(task => {
            ...
            const opacityClass = transparent 
                ? (isFocused ? 'opacity-100 bg-white scale-[1.02] shadow-xl border-[#78b159] z-10' : 'opacity-50 blur-[0.5px] grayscale-[0.3] scale-95 border-transparent') 
                : (task.completed ? 'opacity-60 bg-[#f1f2f6] border-transparent' : 'bg-white opacity-100 border-[#f1f2f6] hover:border-[#78b159] hover:shadow-md');
                ...
                <div key={task.id} className={`group flex items-start gap-3 p-3 rounded-2xl border-2 transition-all duration-500 ${opacityClass}`}>
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                            ...
                            {/* Date Rendering */}
                            {task.dueDate && (
                                <span className="text-[9px] font-bold text-[#78b159] bg-[#f0fff4] px-2 py-0.5 rounded-md border border-[#78b159]/20 flex items-center gap-1">
                                    <CalendarDays size={10} /> {new Date(task.dueDate + 'T00:00').toLocaleDateString(undefined, {month:'short', day:'numeric'})}
                                </span>
                            )}
                    ...
                    <button 
                        onClick={() => !task.completed && setFocusedTaskId(isFocused ? null : task.id)} 
                        className={`p-1.5 rounded-xl transition-colors ${isFocused ? 'bg-[#78b159] text-white shadow-md' : 'text-[#a4b0be] hover:bg-[#f1f2f6]'} ${task.completed ? 'opacity-0 pointer-events-none' : ''}`}
                    >
                        <Target size={16} />
                    </button>
