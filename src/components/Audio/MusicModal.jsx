...
const MusicModal = ({ onClose }) => {
  ...
  // Hardcoded mixer sounds (consistent with your assets)
  const sounds = [
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm animate-pop-in">
        ...
        <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-4xl h-[80vh] flex flex-col overflow-hidden">
             ...
             <div className="flex border-b">
                 <button onClick={() => setTab('spotify')} className={`flex-1 p-4 font-bold transition-colors ${tab === 'spotify' ? 'text-[#1DB954] border-b-4 border-[#1DB954] bg-[#fcfcf7]' : 'text-gray-400 hover:bg-gray-50'}`}>Spotify</button>
                 <button onClick={() => setTab('mixer')} className={`flex-1 p-4 font-bold transition-colors ${tab === 'mixer' ? 'text-[#54a0ff] border-b-4 border-[#54a0ff] bg-[#fcfcf7]' : 'text-gray-400 hover:bg-gray-50'}`}>Mixer</button>
             </div>
             ...
             <div className="flex-1 p-6 bg-[#fcfcf7] overflow-y-auto">
                     ...
                     <div className="flex gap-4 h-full">
                         <div className="w-1/3 space-y-2 overflow-y-auto">
                             {playlists.length > 0 ? playlists.map(p => (
                                 <button key={p.id} onClick={() => setCurrentUrl(p.url)} className="block w-full text-left p-3 rounded-xl bg-white border hover:bg-gray-50 font-bold text-[#594a42] shadow-sm active:scale-95 transition-transform">{p.name}</button>
                             )) : <div className="text-gray-400 text-sm p-2">No playlists found. Add some in settings!</div>}
                         </div>
                         <div className="flex-1 bg-black rounded-xl overflow-hidden shadow-inner">
                             {currentUrl ? <iframe src={currentUrl} width="100%" height="100%" frameBorder="0" allow="encrypted-media"></iframe> : <div className="text-white/50 flex flex-col items-center justify-center h-full gap-2"><Volume2 size={32} /><p>Select a playlist to start listening</p></div>}
                         </div>
                     </div>
                 )}
                 ...
                 {tab === 'mixer' && (
                     <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                         {sounds.map(s => (
                             <div key={s.id} className="bg-white p-6 rounded-2xl border flex flex-col items-center gap-4 shadow-sm">
                                 <div className={`p-4 rounded-full transition-colors ${volumes[s.id] > 0 ? 'bg-[#54a0ff] text-white' : 'bg-gray-100 text-gray-400'}`}><s.icon size={32}/></div>
